...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.task_begin_time = get_time_us();
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            inner.tasks[next].task_begin_time = get_time_us();
    ...
    fn task_begin_time(&self) ->usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let ret = inner.tasks[current].task_begin_time;
        drop(inner);
        ret
    }
    fn current_syscall_times(&self) ->[u32; MAX_SYSCALL_NUM]{
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let mut ret = inner.tasks[current].syscall_times;
        drop(inner);
        ret[current] += 1;
        ret
    }
    fn id_status(&self) -> TaskStatus {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
...
/// task_status
pub fn syscall_status() -> TaskStatus
{
    TASK_MANAGER.id_status()
}
/// syscall_times
...
/// Get start time
pub fn time_begin() -> usize{
    TASK_MANAGER.task_begin_time()
}
/// Run the first task in task list.
